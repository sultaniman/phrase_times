defmodule PhraseTimezonesWeb.Components.AddTimezone do
  ...
  def render(assigns) do
    ...
    <form class="mt-10" phx-change="search" phx-submit="search" phx-target={@myself}>
      <input type="text" name="city_name" value={@city_name} placeholder="Find city///"
  ...
  def suggestion_list(assigns) do
    ...
    <div class="w-[75%] ml-1 mt-2 p-1">
      <%= for city <- @items do %>
      <a phx-click="add_city" phx-target={@myself} phx-value-city-id={city.id} class="flex items-center px-4 py-3 -mx-2 rounded-md transition-colors duration-300 transform hover:bg-slate-50 dark:hover:bg-slate-300">
          <p class="mx-2 text-sm text-gray-600"><span class="font-bold">
          <%= city.city_name %> </span> at <%= city.tz %>
